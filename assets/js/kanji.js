/* Kanji game (simple structure like colors.js) */
...
function setChoicesForNextChar(){
  ...
  const dummies = sampleExcluding(HIRAGANA_POOL, [correct], 5);
...
function startQuestion(){
  ...
  readingPos = 0;
  revealed = Array.from({length: entry.reading.length}).map(()=>false);
  if(el.kanji) {
    el.kanji.textContent = entry.kanji;
    el.kanji.style.transform = `scale(${scale})`;
  }
...
function tick(now){
  ...
  scale += speed * dt;
  if(el.kanji) el.kanji.style.transform = `scale(${scale})`;
  if(scale >= MAX_SCALE){ updateBar(); gameOver(); return; }
  updateBar();
}

async function init() {
  el.kanji = document.getElementById('kanji');
  ...
  try {
    ...
    console.error('kanji.json load failed', e);
    if(el.kanji) el.kanji.textContent = 'データの読み込みに失敗しました。';
    return;
  }

  const tags = getQueryTags();
  pool = tags.length ? data.filter(x => (x.tags||[]).some(t=>tags.includes(t))) : data.slice();
  if(!pool.length){ if(el.kanji) el.kanji.textContent = '該当する問題が見つかりません。ジャンル選択に戻ってください。'; return; }
  shuffle(pool);
